...
FILE *fp1, *fp2; 
int EXPANDING, NT_index = -1, DT_index = -1, NT_curr, lineIndex;   
...
void GETLINE() { 
    if (EXPANDING == 1) { 
        ++lineIndex; 
        strcpy(label, DT[lineIndex].label); 
        strcpy(opcode, DT[lineIndex].opcode); 
        strcpy(operand, DT[lineIndex].operand); 
    } else { 
